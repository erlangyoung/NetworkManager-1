                                  ...
                                  struct nlattr *peer_attr)
{
	...
	if (   peer_c
		...
		if (tb[WGPEER_A_LAST_HANDSHAKE_TIME]) {
			...
			else if (   sizeof (struct timespec) < sizeof (NMPTimespec64)
			         && nla_len (tb[WGPEER_A_LAST_HANDSHAKE_TIME]) >= sizeof (struct timespec)) {
				struct timespec ts;

				/* older versions used "struct timespec". */
				nla_memcpy (&ts, tb[WGPEER_A_LAST_HANDSHAKE_TIME], sizeof (ts));
				peer_c->data.last_handshake_time = (NMPTimespec64) {
					.tv_sec  = ts.tv_sec,
					.tv_nsec = ts.tv_nsec,
				};
			}
