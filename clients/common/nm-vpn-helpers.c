                         ...
                         GError **error)
{
	 ...
	 * NetworkManager will apply a similar configuration when activating the profile. */
	...
	while (line_remainder[0] != '\0') {
		...
		if (line_context == LINE_CONTEXT_INTERFACE) {
			...
			if (line_match (line, "MTU", &matched_key, &value)) {
				...
				/* TODO: not yet implemented. Ignore the value.
				 *
...
fail_invalid_secret:
	...
	for (is_v4 = 0; is_v4 < 2; is_v4++) {
		...
		if (data_table == _TABLE_AUTO) {
			...
			/* TODO: not implemented. Well, currently we don't support adding direct routes for the
			 * allowed-ips ranges. So, it implicitly is implemented.  */
		} else {
			/* TODO: setting NM_SETTING_IP_CONFIG_ROUTE_TABLE to a routing table is not exactly the same as
			 * what wg-quick does. */
			g_object_set (s_ip,
